        ...
        <div className="max-w-6xl mx-auto px-4 py-2.5 flex items-center justify-between gap-4">
          <Link href="/community" className="flex items-center gap-2 flex-shrink-0">
            <div className="w-6 h-6 bg-violet-600 rounded flex items-center justify-center">
              <span className="text-slate-900 text-[10px] font-bold">C</span>
            </div>
            <span className="font-display text-slate-900 text-sm font-semibold hidden sm:block">Communauté</span>
          </Link>
            ...
            {NAV.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-slate-400 hover:text-slate-800 hover:bg-slate-50 transition-all text-xs flex-shrink-0">
