...
export default async function ObserverPage({ params }: Props) {
  ...
  const { data: myProfile } = await supabase
    .from('profiles').select('id, organization_id, first_name').eq('workos_user_id', user.id).maybeSingle();
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <div className="text-center space-y-2 p-8">
          <p className="text-white font-display text-xl">Session non disponible</p>
  ...
  if (!participantIds.includes(myProfile.id)) {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <div className="text-center space-y-2 p-8">
          <p className="text-white font-display text-xl">Accès refusé</p>
  ...
  return (
    <div className="min-h-screen bg-bg">
      <header className="border-b border-white/[0.04] bg-bg-card px-4 py-3 sticky top-0 z-40">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-violet-600 rounded-md flex items-center justify-center">
              <span className="text-white text-[10px] font-bold">TdT</span>
            </div>
            <span className="text-white text-sm font-semibold">Tour de Table · Q{session.quarter} {session.year}</span>
          </div>
