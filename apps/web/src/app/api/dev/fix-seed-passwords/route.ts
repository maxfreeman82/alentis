...
const DOMAINS: string[]       = ['gmail.com','yahoo.fr','outlook.com','hotmail.fr','icloud.com'];
const FAMILIES: string[]      = ['pilotes','initialiseurs','accomplisseurs','dynamiseurs','regulateurs'];
const ENERGY_LEVELS: string[] = ['C1','C2','C3','C4','C5'];

/** Accès tableau garanti sans undefined (noUncheckedIndexedAccess safe) */
...
interface CreatedSpec extends Spec {
...
export async function GET() {
  ...
  const specs: Spec[] = [];
  for (let i = 1; i <= 20; i++) {
    const orgId: string = pick(sortedOrgIds, i - 1);

    const afn = pick(FIRST_NAMES, (i * 3 - 1) % 40);
    const aln = pick(LAST_NAMES,  (i * 2 - 1) % 32);
    specs.push({
      email: `${ascii(afn)}.${ascii(aln)}.adm${i}@${pick(DOMAINS, i % 5)}`,
      password: 'Admin2026!',
      fn: afn, ln: aln, orgId, orgIndex: i, empIndex: 0, isAdmin: true,
    });

    for (let j = 1; j <= 5; j++) {
      const efn = pick(FIRST_NAMES, (i + j * 7 - 1) % 40);
      const eln = pick(LAST_NAMES,  (i * j + 2) % 32);
      specs.push({
        email: `${ascii(efn)}.${ascii(eln)}${i}${j}@${pick(DOMAINS, j % 5)}`,
  ...
  for (let b = 0; b < specs.length; b += BATCH) {
    ...
    const results = await Promise.all(batch.map(async (spec): Promise<CreatedSpec> => {
  ...
  const profileInserts = created
    .filter((u): u is CreatedSpec & { userId: string } => u.userId !== undefined)
    .map(u => ({
      user_id:              u.userId,
      email:                u.email,
  ...
  if (profileErr)
    return NextResponse.json({
      users_created:  created.filter(u => u.userId !== undefined).length,
      profiles_error: profileErr.message,
      auth_errors:    created
        .filter(u => u.authError !== undefined)
        .map(u => ({ email: u.email, error: u.authError })),
    }, { status: 500 });
  ...
  const passportInserts = created
    .filter((u): u is CreatedSpec & { userId: string } => !u.isAdmin && u.userId !== undefined)
    .flatMap(u => {
      ...
      return [{
        ...
        dominant_family: pick(FAMILIES,      (i + j - 1) % 5),
        energy_level:    pick(ENERGY_LEVELS, (i + j)     % 5),
  ...
  return NextResponse.json({
    ...
    users_created:      created.filter(u => u.userId !== undefined).length,
    users_errors:       created.filter(u => u.authError !== undefined).length,
